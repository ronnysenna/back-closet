...
export const createOrder = async (req, res) => {
    try {
        const { total, paymentMethod, shippingAddress, shippingMethod, shippingCost, items, } = req.body;
        ...
        const updatedOrder = await prisma.order.update({
            ...
            data: {
                ...
                notes: existingOrder.notes
                    ? `${existingOrder.notes}\n[${new Date().toISOString()}] Pedido cancelado ${isAdmin ? "pelo administrador" : "pelo cliente"}.`
                    : `[${new Date().toISOString()}] Pedido cancelado ${isAdmin ? "pelo administrador" : "pelo cliente"}.`,
            },
        });
        res.json({
